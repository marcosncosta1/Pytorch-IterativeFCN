...
# logging.basicConfig(level=logging.info()) #was producing an error message
...
def crop_unref_vert(path, out_path, subset):
    ...
    img_names = [f for f in os.listdir(img_path) if f.endswith('.nii.gz')] #changed from .mhd

    for img_name in img_names:
        logging.info('Cropping non-reference vertebrae of %s' % img_name)
        img_name = img_name
        mask_name = img_name.split('.')[0] + '_label.nii.gz' # changed from _label.mhd
...
def calculate_weight(isotropic_path, subset):
    ...
    for f in [f for f in os.listdir(mask_path) if f.endswith('.nii.gz')]: #changed from .mhd
